...
export const Sidebar = ({ className }: IProps) => {
    ...
    return (
        <div className={classNames(classes.sidebar, { [classes.opened]: isOpen }, [className])} data-testid="sidebar">
            ...
            <Button square theme={EThemeButton.BACKGROUND_INVERTED} data-testid="sidebar-toggle-button" type="button" className={classes.togglebutton} onClick={toggleHandler}>{isOpen ? '<' : '>'}</Button>
        </div>
    );
};
